...
Максимальный чек-лист для вывода в работу платы Zynq-7010 (EBAZ4205 или S9) на основе примеров из [repos-vs-tutorial.md](../repo/repos-vs-tutorial.md). Для минимального старта достаточно подмножества; пункты помечены по уровню.
...
Почти все туториалы по EBAZ4205 и Antminer S9 сводятся к одной цепочке:

1. **Vivado**: Block Design PS7 (+ опционально PL) → bitstream → экспорт **XSA**
2. **PetaLinux / Buildroot / SDK**: FSBL + U-Boot + kernel + DTS → **BOOT.BIN**, **image.ub** / **zImage** + **.dtb**, **rootfs**
3. **SD-карта**: FAT-раздел (`BOOT.BIN`, `boot.scr` / `uEnv.txt`, ядро) + ext4 (rootfs) или один `.img` / `.wic`
4. **Boot mode**: джамперы / резисторы → SD / NAND / JTAG

### Типичные подводные камни

| Проблема | Где встречается | Что проверить |
|----------|-----------------|---------------|
| **256 MB RAM** (EBAZ) | U-Boot, `boot.scr` | Адрес загрузки ядра: `0x6000000` vs `0x8000000` |
| **Ethernet без кварца Y3** | EBAZ4205 | Clock из PL через bitstream; без bitstream сеть «мертва» |
| **Варианты платы** | EBAZ, S9 | Нет SD-слота, D24, Y3/X5 → другой DTS/BD |
| **Тишина на UART** | SD boot | Ошибка в `bootgen`/BIF → неверный bootrom header в **BOOT.BIN** |
| **NAND boot log как эталон** | Майнеры | Не копируется на SD без правок U-Boot/DTS |
| **`phy-mode` в DTS** | blkf2016, polprog | На EBAZ PHY обычно **MII**, в DTS часто **rgmii-id** |
| **Брак платы** | S9 с AliExpress/eBay | Проверить SD boot на заведомо рабочей плате |

Подробнее по платам: [EBAZ4205 — ресурсы](../links/EBAZ4205%20-%20ресурсы.md), [Antminer — ресурсы](../links/Antminer%20-%20ресурсы.md). Куда идти за готовым BSP — [Практическая карта](Практическая%20карта.md).
